...
rows = conn.execute("SELECT id, status, hash, created_at FROM design_versions ORDER BY id").fetchall()
for r in rows:
...
rows = conn.execute("SELECT id, version_id, algorithm, created_at FROM optimization_runs ORDER BY id").fetchall()
for r in rows:
