...
def test_all_edges_ok_feasible(tmp_path):
    ...
    vid = _seed(db, board_w=85.0, board_h=56.0, components=[
...
def test_courtyard_margins_included_in_committed(tmp_path):
    ...
    init(db2)
    vid2 = _seed(db2, board_w=85.0, board_h=56.0, components=[
...
def test_keep_outs_reduce_usable_space(tmp_path):
    ...
    vid = _seed(db, board_w=85.0, board_h=56.0,
        keep_outs=[
...
def test_all_four_edges_overcommitted(tmp_path):
    db = _make_db(tmp_path)
    vid = _seed(db, board_w=20.0, board_h=20.0, components=[
...
def test_rpi4_scenario_detects_top_left_conflict(tmp_path):
    ...
    for edge in ("top", "bottom", "left", "right"):
        assert result["edges"][edge]["ok"] is True, \
            f"Edge '{edge}' unexpectedly overcommitted: {result['edges'][edge]}"
